...
class TaskTest < ActiveSupport::TestCase
  ...
  def test_values_of_created_at_and_updated_at
    task = Task.new(title: "test task", assigned_user: @user, task_owner: @user)
    ...
    task.update!(title: "test task updated")
  ...
  def test_task_should_not_be_valid_without_assigned_user
  ...
  def test_task_slug_is_parametrized_title
    title = @task.title
    @task.save!
  ...
  def test_error_raised_when_for_duplicate_slug
  ...
  def test_updating_title_does_not_update_slug
    assert_no_changes -> { @task.reload.slug } do
      updated_task_title = "updated_task_title"
  ...
  def test_having_numbered_slug_substring_in_title_doesnt_affect_slug_generation
    title_with_numbered_substring = "buy 2 apple"
